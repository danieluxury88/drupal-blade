...
/**
 * Lists content types that have entity reference fields.
 *
 * @AuditReport(
 *   id = "content_type_reference_fields",
 *   label = @Translation("Content type reference fields"),
 *   description = @Translation("Shows content types that have entity_reference or entity_reference_revisions fields, including their targets.")
 * )
 */
class ContentTypeReferenceFieldsReport extends AuditReportBase implements ContainerFactoryPluginInterface {
   ...
   *           'field_edit_path' => '/admin/structure/types/manage/article/fields/node.article.field_image',
   *         ],
   *       ],
   *     ],
   *   ],
    ...
    foreach ($node_bundles as $bundle => $info) {
      ...
      foreach ($field_definitions as $field_name => $definition) {
        ...
        $reference_fields[$field_name] = [
          ...
          'field_edit_path' => "/admin/structure/types/manage/$bundle/fields/node.$bundle.$field_name",
        ];
      }

      // Only include content types that have at least one reference field.
  ...
  public function buildRender(array $data): array {
    ...
    if (empty($content_types)) {
      $build['empty'] = [
        '#markup' => $this->t('No content types with entity reference fields were found.'),
      ];
      return $build;
    }
    ...
    // Sort in PHP.
    uasort($content_types, static function (array $a, array $b) use ($order, $sort): int {
      $av = $a[$order] ?? NULL;
      $bv = $b[$order] ?? NULL;
    ...
    foreach ($content_types as $bundle => $info) {
    ...
    $build['content_types'] = [
      '#type' => 'details',
      '#title' => $this->t('Content types with entity reference fields'),
      '#open' => TRUE,
      'table' => [
        '#type' => 'table',
        '#header' => $header,
        '#rows' => $rows,
        '#empty' => $this->t('No content types with entity reference fields were found.'),
      ],
    ];
  ...
  public function buildMarkdown(array $data): string {
    ...
    if (empty($content_types)) {
      return "No content types with entity reference fields were found.\n";
    }

    $lines = [];

    $lines[] = '# Content type reference fields';
    $lines[] = '';
